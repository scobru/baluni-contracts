    ...
    IPlatypusPool
{
    ...
    IBaluniV1Oracle private _priceOracle;
    ...
    function initialize(address _registry) external initializer {
        ...
        registry = _registry;
        _priceOracle = IBaluniV1Oracle(IBaluniV1Registry(registry).getBaluniOracle());
        USDC = IBaluniV1Registry(registry).getUSDC();
    ...
    function getPriceOracle() external view returns (address) {
        return IBaluniV1Registry(registry).getBaluniOracle();
    }
     ...
     */
    function setSlippageParams(uint256 k_, uint256 n_, uint256 c1_, uint256 xThreshold_) external onlyOwner {
     ...
     */
    function setPriceOracle() external onlyOwner {
        _priceOracle = IBaluniV1Oracle(IBaluniV1Registry(registry).getBaluniOracle());
        emit OracleUpdated(address(_priceOracle), address(_priceOracle));
    }
    ...
    function _checkPriceDeviation(address tokenA, address tokenB) private view {
        IBaluniV1Oracle oracle = IBaluniV1Oracle(IBaluniV1Registry(registry).getBaluniOracle());

        uint256 unitA = 10 ** _getAsset(tokenA).decimals();
        uint256 unitB = 10 ** _getAsset(tokenB).decimals();

        uint256 tokenAPrice = oracle.convert(tokenA, USDC, unitA);
        uint256 tokenBPrice = oracle.convert(tokenB, USDC, unitB);
    ...
    function _getEquilibriumCoverageRatio() private view returns (uint256) {
        ...
        for (uint256 i; i < _sizeOfAssetList(); ++i) {
            ...
            uint256 tokenPrice = _priceOracle.convert(assetAddress, USDC, 10 ** _getAsset(assetAddress).decimals());
    ...
    function _withdrawFrom(
        Asset asset,
        uint256 liquidity
    ) private view returns (uint256 amount, uint256 liabilityToBurn, uint256 fee, bool enoughCash) {
    ...
    function quotePotentialWithdraw(
        address token,
        uint256 liquidity
    ) external view override whenNotPaused returns (uint256 amount, uint256 fee, bool enoughCash) {
        require(token != address(0), 'ZERO');
    ...
    function quoteMaxInitialAssetWithdrawable(
        address initialToken,
        address wantedToken
    ) external view whenNotPaused returns (uint256 maxInitialAssetAmount) {
        _checkPriceDeviation(initialToken, wantedToken);
