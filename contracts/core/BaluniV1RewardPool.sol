...
contract BaluniV1RewardPool is ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  /// @param _stakedToken BALUNI token address
  function initialize(address _stakedToken) external initializer {
    __ERC20_init('BALUNI reward pool', 'rBALUNI');
    __Ownable_init();
  ...
  function stakeWithPermit(
    ...
    IERC20PermitUpgradeable(address(stakedToken)).permit(_user, address(this), _amount, _deadline, _v, _r, _s);
    _stake(_user, _amount);
  }

  /// @notice Withdraw BALUNI tokens
  /// @dev Burns an equal number of receipt tokens from the caller
  /// @param _amount Amount of BALUNI to withdraw
  function withdraw(uint256 _amount) external update(msg.sender) {
    _withdraw(_amount);
  }

  /// @notice Withdraw all of the caller's BALUNI tokens and claim rewards
  ...
  function earned(address _user) external view returns (address[] memory rewardTokens, uint256[] memory earnedAmounts) {
  ...
  function rewardInfo(
    ...
    external
    view
    returns (address reward, uint256 periodFinish, uint256 duration, uint256 lastUpdateTime, uint256 rate)
  {
    reward = rewards[_rewardId];
    RewardInfo storage info = _getRewardInfo(reward);
    (periodFinish, duration, lastUpdateTime, rate) = (info.periodFinish, info.duration, info.lastUpdateTime, info.rate);
  }
  ...
  /// @dev Stake BALUNI tokens and mint the caller receipt tokens
  ...
  /// @dev Withdraw BALUNI tokens and burn an equal number of receipt tokens from the caller
  /// @param _amount Amount of BALUNI to withdraw
  ...
  /// @dev Calculate the reward amount per BALUNI token
  /// @param _reward Address of the reward
  /// @return rewardPerToken Reward amount per BALUNI token
  function _rewardPerToken(address _reward) private view returns (uint256 rewardPerToken) {
    ...
    if (totalSupply() == 0) {
      ...
      rewardPerToken =
        ...
        (((_lastTimeRewardApplicable(rewardData.periodFinish) - rewardData.lastUpdateTime) * rewardData.rate * 1e18) /
          totalSupply());
    }
  }
