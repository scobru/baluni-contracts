...
contract BaluniV1Stake is ReentrancyGuard {
  ...
  function updateRewardIndex(uint256 reward) internal {
    if (stakingSupply > 0) {
      rewardIndex = rewardIndex + ((reward * (MULTIPLIER)) / (stakingSupply));
    }
  }
  ...
  function _calculateRewards(address account) private view returns (uint256) {
    ...
    return (shares * (timeStaked) * (rewardDelta)) / (MULTIPLIER) / (STAKING_PERIOD);
  }
  ...
  function claimTo(address staker, address _to) public nonReentrant returns (uint256) {
